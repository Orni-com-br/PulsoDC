...
import {
  ArrowLeft, Plus, Search, FileText, SprayCan, Shirt, Package,
  TrendingUp, TrendingDown, AlertTriangle, Edit2, Trash2, X, Save, Download
} from "lucide-react";
...
type Categoria = "expediente" | "higiene" | "fardamento";
...
const catConfig: Record<Categoria, { label: string; icon: typeof FileText; color: string; bg: string }> = {
  expediente: { label: "Expediente", icon: FileText, color: "text-blue-600", bg: "bg-blue-50 border-blue-200" },
  higiene: { label: "Higiene e Limpeza", icon: SprayCan, color: "text-purple-600", bg: "bg-purple-50 border-purple-200" },
  fardamento: { label: "Fardamento", icon: Shirt, color: "text-amber-600", bg: "bg-amber-50 border-amber-200" },
};

export default function EstoqueAdministrativo() {
  ...
  const [form, setForm] = useState({ nome: "", categoria: "expediente" as Categoria, quantidade: 0, unidade: "un", minimo: 0 });
  ...
  const { data: items = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("estoque_administrativo").select("*").order("nome");
  ...
  const saveMutation = useMutation({
    mutationFn: async (item: Partial<ItemEstoque>) => {
      if (editItem) {
        const { error } = await supabase.from("estoque_administrativo").update({ ...item, ultima_atualizacao: new Date().toISOString() }).eq("id", editItem.id);
        if (error) throw error;
      } else {
        const { error } = await supabase.from("estoque_administrativo").insert([{ ...item, ultima_atualizacao: new Date().toISOString() }]);
        if (error) throw error;
      }
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["estoque_administrativo"] });
      setDialogOpen(false);
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from("estoque_administrativo").delete().eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["estoque_administrativo"] });
  ...
  const movMutation = useMutation({
    mutationFn: async ({ id, quantidade }: { id: string, quantidade: number }) => {
      const { error } = await supabase.from("estoque_administrativo").update({ quantidade, ultima_atualizacao: new Date().toISOString() }).eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["estoque_administrativo"] });
  ...
  function openNew() {
    setEditItem(null);
    setForm({ nome: "", categoria: "expediente", quantidade: 0, unidade: "un", minimo: 0 });
    setDialogOpen(true);
  }
  ...
  function exportCSV() {
    ...
    link.href = url;
    link.download = "estoque_administrativo.csv";
    link.click();
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      <div className="bg-card rounded-2xl shadow p-6 sm:p-8">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-2xl font-bold">Estoque Administrativo</h2>
              <p className="text-muted-foreground text-sm">Materiais de expediente, higiene e fardamento</p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <KpiCard icon={<Package className="w-5 h-5 text-blue-500" />} label="Itens cadastrados" value={items.length} />
        <KpiCard icon={<TrendingUp className="w-5 h-5 text-emerald-500" />} label="Total em estoque" value={totalItens} />
        <KpiCard icon={<AlertTriangle className="w-5 h-5 text-amber-500" />} label="Abaixo do mínimo" value={alertas} alert={alertas > 0} />
        <KpiCard icon={<Shirt className="w-5 h-5 text-purple-500" />} label="Categorias" value={3} />
      </div>
      ...
      <div className="bg-card rounded-2xl shadow p-4 flex flex-col sm:flex-row gap-3">
        ...
        <div className="flex gap-2 flex-wrap">
          {(["todos", "expediente", "higiene", "fardamento"] as const).map((cat) => (
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4 pt-2">
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <Button className="rounded-xl" onClick={saveItem}>
